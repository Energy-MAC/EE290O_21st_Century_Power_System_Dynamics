function dxdt = ode_full_system_modular_conv(t,x, u, param, pl)
...
%Converters
inv_Cdc_convs = param.inv_Cdc_convs;
...
iac_convs = x(pl.iac_convs_init: pl.iac_convs_end);
...
theta_convs = x(pl.theta_convs_init : pl.theta_convs_end);
...
mu_convs = u(pl.mu_convs_init : pl.mu_convs_end);


%% Compute the derivatives using auxiliary functions

% diff_i_gens = di_gens_dt(i_gens, v_buses, theta_gens, omega_gens, i_f_gens, Z_gens, inv_L_gens, I_inc_gens,  Ell_gens); %Eq (4c)

diff_i_convs = di_convs_dt(v_buses, iac_convs, theta_convs, vdc_convs, mu_convs, Zac_convs, inv_Lac_convs, I_inc_convs);
...
i_in = -I_inc_convs*iac_convs - I_inc_loads*i_loads - I_inc_infbus*i_infbus; %Port variables w/o generator
%i_in = -I_inc_convs*iac_convs - I_inc_loads*i_loads; %Port variables w/o generator or inf bus
%i_in is defined as postive into the network
...
diff_theta_convs = dtheta_convs_dt(vdc_convs, omega0, param.kmc_convs, param.vdcref_convs); %Eq (24c) and 25 Curi
diff_vdc_convs = dvdc_convs_dt(idc_convs, iac_convs, theta_convs, vdc_convs, mu_convs, Gdc_convs, inv_Cdc_convs, param.vdcref_convs); %Eq (7a) Curi
