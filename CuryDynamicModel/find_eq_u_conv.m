function [x_eq,u_free] = find_eq_u_conv(x0,Mass_Matrix,u0,u_free_ind,param,param_limits)
...
x_eq(~ind) = find_ac_conv(x_eq,Mass_Matrix,u_eq,param,param_limits);
...
function y = f(x_dc_and_u)
    ...
    % converge
    t_dc(~ind) = find_ac_conv(t_dc,Mass_Matrix,u,param,param_limits);
    
    % Compute derivative given current iteration dc variables (x_dc) and
    % the associated steady state AC variables.
    y = ode_full_system_modular_conv(0,t_dc,u,param, param_limits);
