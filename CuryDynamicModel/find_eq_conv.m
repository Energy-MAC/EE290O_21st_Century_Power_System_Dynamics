function x_eq = find_eq_conv(x0,Mass_Matrix,u,param,param_limits)
...
x_eq(~ind) = find_ac_conv(x_eq,Mass_Matrix,u,param,param_limits);
...
function y_dc = f(x_dc)
    ...
    % converge
    t(~ind) = find_ac_conv(t,Mass_Matrix,u,param,param_limits);
    
    % Compute derivative given current iteration dc variables (x_dc) and
    % the associated steady state AC variables.
    y_dc = ode_full_system_modular_conv(0,t,u,param, param_limits);
